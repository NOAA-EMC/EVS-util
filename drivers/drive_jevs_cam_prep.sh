...
if [[ "$prep_job" == *"rrfsmem"* ]]; then
    for mem in {1..5}; do
        qsub -v vhr=$vhr,mem=$mem ${drivers_dir}/jevs_${STEP}_${COMPONENT}_${prep_job}.sh
    done
else
    qsub -v vhr=$vhr ${drivers_dir}/jevs_${STEP}_${COMPONENT}_${prep_job}.sh
fi
